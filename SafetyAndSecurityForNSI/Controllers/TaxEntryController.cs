...
namespace SafetyAndSecurityForNSI.Controllers
{
    ...
    public class TaxEntryController : Controller
    {
        ...
        // GET: TaxEntry
        public ActionResult Index(int?page,Guid? TaxId)
        {
            int pageNo = 0;
            pageNo = page == null ? 1 : int.Parse(page.ToString());
            var allList = db.TaxEntryModels.ToList();
            if (TaxId != null)
            {
                allList = allList.Where(m => m.TaxId == TaxId).ToList();
            }
            int allCount = db.TaxEntryModels.Count();
            int dataPerPage = 20;
            int dataPerPageEnd = pageNo * dataPerPage;
            int dataPerPageStart = dataPerPageEnd - dataPerPage;
            var data = allList.OrderBy(e => e.TaxId).Skip(dataPerPageStart).Take(dataPerPage);
            Pager<TaxEntryModel> pager = new Pager<TaxEntryModel>(data.AsQueryable(), pageNo, dataPerPage, allCount);
            return View(pager);
        }
        public JsonResult SearchData(long searchId)
        {
            ...
            {
                var data = db.TaxEntryModels.Where(m => m.NID == searchId || m.ITIN == searchId).Distinct().ToList();
