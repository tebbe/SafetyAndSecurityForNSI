...
namespace SafetyAndSecurityForNSI.Controllers
{
    ...
    public class HomeLoanController : Controller
    {
        ...
        // GET: HomeLoan
        public ActionResult Index(int?page,Guid? HomeLoanId)
        {
            int pageNo = 0;
            pageNo = page == null ? 1 : int.Parse(page.ToString());
            var allList = db.HomeLoanModels.ToList();
            if (HomeLoanId != null)
            {
                allList = allList.Where(m => m.HomeLoanId == HomeLoanId).ToList();
            }
            int allCount = db.HomeLoanModels.Count();
            int dataPerPage = 20;
            int dataPerPageEnd = pageNo * dataPerPage;
            int dataPerPageStart = dataPerPageEnd - dataPerPage;
            var data = allList.OrderBy(e => e.HomeLoanId).Skip(dataPerPageStart).Take(dataPerPage);
            Pager<HomeLoanModel> pager = new Pager<HomeLoanModel>(data.AsQueryable(), pageNo, dataPerPage, allCount);
        ...
        public JsonResult SearchData(long searchId)
        {
            ...
            {
                var data = db.HomeLoanModels.Where(m => m.NID == searchId || m.BirthId == searchId).Distinct().ToList();
